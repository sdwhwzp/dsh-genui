...
// `content` is what saved itinerary replies carry for the step body; the
// record otherwise renders without its text.
const stepsRecordSchema = recordSchema(['title'], { title: 'string', desc: 'string' }, {}, {}, { content: 'desc' })
...
// Models name the file `file`/`filePath`/`filename` and the two sides
// `new`/`after` and `old`/`before`; a record carrying only a unified-diff
// string is split in `normalizeDiffRecords`.
const diffRecordSchema = recordSchema(['path', 'newText'], { path: 'string', oldText: 'string-or-null', newText: 'string' }, {}, {}, {
  file: 'path', filePath: 'path', filename: 'path', new: 'newText', after: 'newText', old: 'oldText', before: 'oldText',
})
...
export const COMPONENT_SCHEMAS: Readonly<Record<string, ComponentSchema>> = {
  ...
  diff: schema(['diffs'], { ...nodeFields, diffs: 'array' }, { items: 'diffs', files: 'diffs', diff: 'diffs', changes: 'diffs', content: 'diffs', text: 'diffs' }, { nested: { diffs: diffRecordSchema } }),
  ...
  hero: schema(['title'], {
    ...
    spark: 'array',
    tone: 'string',
  }, { number: 'value' }, { enums: { tone: HERO_TONES }, valueAliases: { tone: { brand: 'accent' } } }),
